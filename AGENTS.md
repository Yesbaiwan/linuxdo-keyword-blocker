...
- 面板：总开关；规则 = 类别下拉 + 标签（每框一个，最多 3 个，仅剩 1 个框时不给 ×）+ 标题，至少填一项，与现有规则重复时状态行提示且表单内容保留；「零标签」复选框在标签行内，勾选后标签框与「+」收起；单规则启停（停用画删除线）；批量启停（页脚左组「清空」旁的降权文字按钮，文字显示将执行的动作——全开显「全部停用」、否则显「全部启用」，可逆无需确认，0 规则时禁用）；行内编辑在本行下方展开，与添加表单同一套字段；任何重建列表的操作（添加 / 删除 / 清空 / 保存、类别树就绪）都会收起编辑表单，只有单规则启停是就地更新；类别下拉外观同普通下拉框，输入即过滤、点选 / 回车选中、× 清除、失焦恢复；候选列表是 fixed 浮层，贴输入框上下方展开；隐藏 / 淡化两模式（默认淡化）；导入 / 导出 JSON（同一格式规则数组，导入为追加模式：文件内部重复、与现有规则重复都跳过，只收新规则，提示里带跳过计数）；清空（页脚左端的降权文字按钮，与右侧导入/导出拉开距离；两段式确认——首击进红色确认态、3 秒无操作自动复位、再击才执行，0 规则时禁用）。已添加规则的搜索刻意不做。
